...
async fn main() -> Result<(), LpmError> {
    ...
    match cli.command {
        WatchCommands::Watch { command, paths, ignore, no_clear, script, websocket_port } => {
            watch::run(command, paths, ignore, no_clear, script, Some(websocket_port)).await
        }
        WatchCommands::Dev { script } => {
            watch::run(None, None, None, false, script, None).await
        }
    }
}
