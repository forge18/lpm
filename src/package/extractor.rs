...
impl PackageExtractor {
    ...
    pub fn extract(&self, archive_path: &Path) -> LpmResult<PathBuf> {
        ...
        let result = match extension {
            ...
            _ => Err(LpmError::Package(format!("Unsupported format: {}", extension))),
        };
        
        // Cleanup temp directory on error
        if result.is_err() {
            let temp_dir = self.dest_dir.join(format!(".tmp-{}", 
    ...
    fn extract_targz(&self, archive_path: &Path) -> LpmResult<PathBuf> {
        ...
        let root = entries.find_map(|e| {
            ...
            LpmError::Package("Archive has no root directory. Files at root level not supported.".to_string())
        })?;
    ...
    fn extract_zip(&self, archive_path: &Path) -> LpmResult<PathBuf> {
        ...
        let mut archive = ZipArchive::new(file)
            .map_err(|e| LpmError::Package(format!("Invalid zip: {}", e)))?;
