...
impl Publisher {
    ...
    pub async fn publish(&self, include_binaries: bool) -> LpmResult<()> {
        ...
        let username = CredentialStore::retrieve("luarocks_username")
            .map_err(|_| LpmError::Package(
                "LuaRocks username not found. Run 'lpm login' first.".to_string(),
            ))?;
        
        let api_key = CredentialStore::retrieve("luarocks_api_key")
            .map_err(|_| LpmError::Package(
                "LuaRocks API key not found. Run 'lpm login' first.".to_string(),
            ))?;
        ...
        let rockspec_path = self.project_root.join(format!("{}-{}.rockspec", 
            self.manifest.name, 
            crate::luarocks::version::to_luarocks_version(
                &crate::core::version::Version::parse(&self.manifest.version)?
            )
        ));
        ...
        self.upload_to_luarocks(&rockspec_path, &archive_path, &username, &api_key).await?;
