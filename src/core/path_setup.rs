...
pub fn check_path_setup() -> LpmResult<()> {
    ...
    if exe_dir_normalized == cargo_bin_normalized || current_exe.starts_with(&cargo_bin) {
        ...
        if cfg!(target_os = "windows") {
            ...
            eprintln!("  [Environment]::SetEnvironmentVariable(\"Path\", $env:Path + \";{}\", \"User\")", cargo_bin.display());
        } else {
            // Unix-like systems
            ...
            eprintln!("\nOr run this command:");
            eprintln!("  echo 'export PATH=\"$HOME/.cargo/bin:$PATH\"' >> {}", profile_file);
...
fn get_cargo_bin_dir() -> LpmResult<PathBuf> {
    if cfg!(target_os = "windows") {
        ...
        let home = env::var("HOME")
            .map_err(|_| LpmError::Path("HOME not set".to_string()))?;
